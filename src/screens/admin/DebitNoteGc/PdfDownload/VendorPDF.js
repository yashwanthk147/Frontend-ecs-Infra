...
const VendorPDF = (props) => {
  ...
  //   let dateVal = new Date(datestr);
  //   return months[dateVal.getMonth() + 1] + "-" + dateVal.getFullYear();
  // };
  ...
  //     new_value = value + ".00";
  //   } else {
  //     new_value = value * 1; //removes trailing zeros
  ...
  //   return new_value;
  // }
  console.log("Debit note details", debitNoteGcDetails);
  return (
    <div id="po_view_main">
      {debitNoteGcDetails !== null && (
        <>
              ...
              <Grid item className="first-section-main-import">
                ...
                {debitNoteGcDetails?.entity_address
                  ...
                  ?.map((item, index) => {
                    return (
                      <>
                        {item.includes("GST NO") ? (
                          <b>{item}</b>
                        ) : item.includes("PAN NO") ? (
                          <b>{item}</b>
                        ) : (
                          item + ","
                        )}
          ...
          <p>
            ...
            <strong>
              {debitNoteGcDetails.debit_amount} (Rupees{" "}
              {convertRupeesIntoWords(
                parseInt(debitNoteGcDetails.debit_amount)
              )}{" "}
              Only){" "}
            </strong>{" "}
            towards weight and quality problems:
          </p>
            ...
            <Table className="pdf_table" aria-label="simple table">
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell className="text-center" rowSpan={3}>
                    {debitNoteGcDetails.vehicleno}
                  </TableCell>
                  <TableCell className="text-center" rowSpan={3}>
                    {debitNoteGcDetails.itemdesc}
                  </TableCell>
                  <TableCell className="text-center">
                    {debitNoteGcDetails.hsc_code}
                  </TableCell>
                  <TableCell className="text-center">KGS</TableCell>
                  <TableCell className="text-center">
                    {Number(debitNoteGcDetails.invoice_qty)?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </TableCell>
                  <TableCell className="text-center">
                    {Number(debitNoteGcDetails.netrecd)?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </TableCell>
                  <TableCell className="text-center">
                    {Number(debitNoteGcDetails.debit_amount)?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </TableCell>
                </TableRow>
                <TableRow>
                  <TableCell className="text-center">
                ...
                <TableRow>
                  <TableCell className="text-center">
                    {Number(debitNoteGcDetails.husk)?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </TableCell>
                  <TableCell className="text-center">
                    {Number(debitNoteGcDetails.stones)?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </TableCell>
                  <TableCell className="text-center">
                    {Number(debitNoteGcDetails.moisture)?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </TableCell>
                  <TableCell className="text-center">
                    {Number(debitNoteGcDetails.quality)?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </TableCell>
                  <TableCell className="text-center"></TableCell>
                </TableRow>
                <TableRow>
