...
const calculateAmount = (data) => {
  ...
  return data?.["purchase_price"] * totalQuantity;
};
    ...
    <View style={styles.table}>
      ...
      {data.map((row, i) => (
        <View key={i} style={styles.row} wrap={false}>
            ...
            <View>
              ...
              <View style={{ flexDirection: 'column' }}>
                ...
                {dispatchDetails?.map((detail, index) => <View key={index} style={{ display: 'flex', flexDirection: 'row' }} wrap={false}>
                  <CustomText style={{ width: '50%' }}>{detail.dispatch_date}</CustomText>
          ...
          <CustomText style={styles.row5}>{Number(row.rate)?.toLocaleString('en-IN', { minimumFractionDigits: 4, maximumFractionDigits: 4 })}</CustomText>
        ...
        >
          <View style={{ flexDirection: "row" }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                {Number(calculateGrossPrice(purchaseDetails))?.toLocaleString('en-IN', { minimumFractionDigits: 4, maximumFractionDigits: 4 })}
              </CustomText>
            </View>
          </View>
      ...
      <Page style={styles.section} size={"A4"}>
              ...
              <View style={{ width: "70%", display: "flex" }}>
                ...
                >
                  ...
                  >
                    ...
                    {purchaseDetails?.supplier_address
                      ?.split(",")
                      // eslint-disable-next-line
                      ?.map((item, index) => {
                        if (item !== "") {
                          return (
                            <>
                              ...
                              {index >=
                                purchaseDetails.supplier_address.split(",")
                                  .length -
                                4 && "\n"}
                            </>
                          );
        ...
        <View>
          ...
          <Conditions
            ...
            value={
              validateValue(purchaseDetails.advance) &&
                parseFloat(purchaseDetails.advance) !== 0
                ? `${parseFloat(purchaseDetails.advance).toFixed(2)}
                        ${purchaseDetails.advance_type === "Percentage"
                  ? "%"
                  : ""
                } :`
                : null
          ...
          <Conditions
            ...
            value={
              validateValue(purchaseDetails.payment_terms_days) &&
              purchaseDetails.payment_terms_days +
              // eslint-disable-next-line
              " " +
              "WORKING DAYS FROM DATE OF INVOICE"
            }
          />
          <Conditions
