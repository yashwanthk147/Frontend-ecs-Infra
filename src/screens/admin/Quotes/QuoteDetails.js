...
const QuoteDetails = (props) => {
    ...
    const getSaveButton = () => {
        ...
        else if (quoteDetails?.status === "Base Price Received" && (role === roles.managingDirector || role === roles.marketingExecutive || role ===roles.BusinessDevelopmentManager)
            && quoteItems?.length > 0
        ...
        else if (quoteDetails?.status === "Quote Submitted" && (role === roles.managingDirector || role === roles.marketingExecutive || role === roles.BusinessDevelopmentManager)
            && quoteItems?.length > 0
            && quoteItems?.reduce((accumulator, current) => {
        ...
        else if (quoteDetails?.status === "Bid Submitted to GMC" && (role === roles.managingDirector || role === roles.gmc || role === roles.BusinessDevelopmentManager)
            && quoteItems?.length > 0
            && quoteItems?.reduce((accumulator, current) => {
    ...
    const quoteItemInfo = () => (
        <Container className={classes.popover}>
            ...
            <QuoteItemList data={quoteItems}
                quoteItemDetails={(event, quoteItemId) => navigate(`/quote-item/${quoteDetails?.accountid}/${quoteItemId}/view?status=${quoteDetails?.status}`)} />
        </Container>
    );
        ...
        <>
            ...
            <Grid container direction="row">
                ...
                {quoteDetails?.status !== "New" ? null : <Grid xs={6} item justify="flex-end" alignItems="center" style={{ display: 'flex' }}>
                    <Fab label={"Create Quote Item"} variant="extended" onClick={() => {
                        navigate(`/quote-item/${quoteDetails?.accountid}/${props.id}/${quoteDetails?.quote_autogennumber}/create?clonedQuoteId=${quoteDetails?.cloned_quoteid}`);
                        // debugger;
                    }
                    } />
            ...
            <Grid container xs={12} md={12} style={{ margin: 24 }} justify="center">
                {role !== roles.managerPurchaseGCQuotes && !props?.hideEdit && (role === roles.marketingExecutive && status !== 'Base Price Requested') &&
                    <Grid item>
                        <Button label="Edit" onClick={(e) => navigate(routeBuilder('quote', props.id, 'edit'))} />
                    </Grid>}
                <Grid item>
